package com.github.zeldazach.binghamtonrover.Monitors;

/*
 * According to the Json File, this class will not be needed, but keep it around just so
 * we might need this in the future.
 * The Gyroscope monitors the angular velocity of the rover along three axis (X,Y,Z)
 * in degrees/radian per seconds.
 */

public class GyroscopeMonitor extends InformationObserver
{

}
