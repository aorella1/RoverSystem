...
// The colors are defined in the YCbCr format.
...
struct CaptureSession {
    ...
    // Waits for the next frame from the camera. Method ''start_stream''
    // MUST be called first. Returns a pointer to a JPEG image on success
    // and nullptr otherwise. Param ''out_size'' is updated with the size
    // of the image.
    size_t grab_frame(uint8_t* out_buffer);
};
