...
CaptureSession::CaptureSession(size_t _width, size_t _height) {
    width = _width;
    height = _height;
}
...
bool CaptureSession::check_capabilities() {
    ...
    if (fmt.fmt.pix.width != width || fmt.fmt.pix.height != height) {
        std::cerr << "[!] Camera does not support requested size!" << std::endl;
        return false;
    }
...
size_t CaptureSession::grab_frame(uint8_t* out_buffer) {
    ...
    // Copy the bytes over.
    memcpy(out_buffer, frame_data, frame_size);
